...
#ifndef __IEXP_RAND_PARETO__
#define __IEXP_RAND_PARETO__
  ...
  public:
    ...
    class rng
    {
      public:
        ...
        double next()
        {
            return gsl_ran_pareto(m_rng.gsl(), m_a, m_b);
        }
    ...
    static inline auto fill(DenseBase<T> &x,
        ...
        -> decltype(x.derived())
    {
        pareto::rng r(a, b, type, seed);
        return fill(x, r);
    }

    template <typename T>
    static inline auto fill(DenseBase<T> &x, pareto::rng &r)
        -> decltype(x.derived())
    {
