...
#ifndef __IEXP_RAND_NEG_BINOMIAL__
#define __IEXP_RAND_NEG_BINOMIAL__
  ...
  public:
    ...
    class rng
    {
      public:
        ...
        unsigned int next()
        {
            return gsl_ran_negative_binomial(m_rng.gsl(), m_p, m_n);
        }
    ...
    static inline auto fill(DenseBase<T> &x,
        ...
        -> decltype(x.derived())
    {
        nbnom::rng r(p, n, type, seed);
        return fill(x, r);
    }

    template <typename T>
    static inline auto fill(DenseBase<T> &x, nbnom::rng &r)
        -> decltype(x.derived())
    {
...
#endif /* __IEXP_RAND_NEG_BINOMIAL__ */
