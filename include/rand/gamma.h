...
#ifndef __IEXP_RAND_GAMMA__
#define __IEXP_RAND_GAMMA__
  ...
  public:
    ...
    class rng
    {
      public:
        ...
        double next()
        {
            return gsl_ran_gamma(m_rng.gsl(), m_a, m_b);
        }
    ...
    static inline auto fill(DenseBase<T> &x,
        ...
        -> decltype(x.derived())
    {
        gamma::rng r(a, b, type, seed);
        return fill(x, r);
    }

    template <typename T>
    static inline auto fill(DenseBase<T> &x, gamma::rng &r)
        -> decltype(x.derived())
    {
