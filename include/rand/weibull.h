...
#ifndef __IEXP_RAND_WEIBULL__
#define __IEXP_RAND_WEIBULL__
  ...
  public:
    ...
    class rng
    {
      public:
        ...
        double next()
        {
            return gsl_ran_weibull(m_rng.gsl(), m_a, m_b);
        }
    ...
    static inline auto fill(DenseBase<T> &x,
        ...
        -> decltype(x.derived())
    {
        wbl::rng r(a, b, type, seed);
        return fill(x, r);
    }

    template <typename T>
    static inline auto fill(DenseBase<T> &x, wbl::rng &r)
        -> decltype(x.derived())
    {
