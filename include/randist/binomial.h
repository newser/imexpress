...
{
  public:
    bnom(const double p, const unsigned int n)
        : m_p(p)
        , m_n(n)
    {
    }
...
class bnom_pdf_functor
{
  public:
    ...
    bnom_pdf_functor(const T &x, const double p, const unsigned int n)
        : m_x(x)
        , m_bnom(p, n)
    {
    }
...
bnom_pdf(const ArrayBase<T> &x, const double p, const unsigned int n)
{
